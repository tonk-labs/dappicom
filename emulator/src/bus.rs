...
impl CpuBus {
    ...
    pub fn new(ram_state: RamState) -> Self {
        ...
        // this won't be captured in the trace, but we really don't care afaik
        // the initial reads from memory are completely arbitrary
        // once a merkle root is committed then we have something
        // but without that the first read is unconstrained
        // however, what we would want is for the ROM space to be constrained initially (to prove that it is reading from correct program)
        // in order to do that, we can set the initial WRAM to be unconstrained, but the ROM space reads to always be in a special ROM merkle tree?
